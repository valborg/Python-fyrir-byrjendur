...
Þá megum við nýta okkur allt að þrjá stika (e. parameters).
...
Í línu 4 eru tveir tvípunktar innan hornklofanna og tölurnar sem koma á milli þeirra er afmörkunin á því hvað við viljum lesa upp úr \texttt{strengur}.
...
# Sækjum einhvern hluta úr strengnum
...
\section{Strengjaaðferðir}\index{Strengjaaðferðir}
...
\begin{enumerate}
	...
	\item strengur.index('x') skilar villu þar sem táknið 'x' kemur ekki fyrir í strengnum
	\item strengur.find('v') skilar tölunni 0 þar sem fyrsta 'v' táknið kemur fyrir í vísi 0
	\item strengur.find('x') skilar tölunni -1 þar sem 'x' finnst ekki í stregnum. 
\end{enumerate}
\vspace{10px}
...
\begin{lstlisting}[caption=Aðferðir kynntar, label=lst:adferdir]
	strengur = "kex með smjöri, osti og sultu"
\end{lstlisting}
\todo{fylla inn í þennan kóðabút, setja inn .format()??}
