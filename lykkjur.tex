...
Fallið \texttt{range()} tekur við sömu viðföngum eins og hornklofarnir\footnote{Það eru í raun ekki viðföng, í "strengur"[1:5:2] eru 1, 5, og 2 ekki viðvöng beinlínis heldur vísar.} þegar við sóttum nokkur tákn upp úr streng eða lista, þau eru þó aðgreind með kommum.
\footnote{Við höfum áður séð viðföng notuð í falli eins og \texttt{print()} fallið, þar sem við getum prentað margt út svo lengi sem við setjum kommur á milli.}
...
Hægt er að stöðva vélina handvirkt í þessum aðstæðum.
\footnote{Í Jupyter Notebooks er það gert með Kernel -> Restart Kernel.}
...
Ef óskað væri eftir að hafa það tákn líka væri hægt að rifja upp \texttt{.lower()} úr kafla \ref{k:strengir} eða hvernig eigi að nota rökvirkja í segð.
\footnote{Þá í stað \texttt{if "a" in stak} væri \texttt{if "a" in stak.lower()}, eða með rökvirkja \texttt{if "a" in stak or "A" in stak}.}
